# 振幅が初めて1,000,000を超えた時の周波数のみを配列に保存するプログラム
    ...
    with open(output_file_name, "w", newline='') as file:
        ...
        for elapsed_time in np.arange(0, total_samples / sample_rate, segment_duration_sec):
            ...
            if max_amplitude > 1000000 or threshold_exceeded:
                ...
                writer.writerow([f"{max_frequency:.2f}"])
...
#output_tsv_path = "../data/test_correct_cutstart7.tsv"  # 任意の出力ファイル名

# FFT処理を実行して結果を配列に保存
resulting_frequencies = process_fft(input_mp3_path, output_tsv_path)

# 振幅の閾値を1000000に設定し、その値を超えた時間、周波数、振幅を出力するプログラム
...
def process_fft(input_file_path, output_file_name, segment_duration_sec=0.25):
    ...
    with open(output_file_name, "w", newline='') as file:
        ...
        writer.writerow(["Elapsed Time (s)", "Frequency (Hz)", "Amplitude"])
        ...
        for elapsed_time in np.arange(0, total_samples / sample_rate, segment_duration_sec):
            ...
            if max_amplitude > 1000000 or threshold_exceeded:
                ...
                print(f"Elapsed Time: {elapsed_time:.2f} seconds, Frequency: {max_frequency:.2f} Hz, Amplitude: {max_amplitude:.2f}")
                writer.writerow([f"{elapsed_time:.2f}", f"{max_frequency:.2f}", f"{max_amplitude:.2f}"])

# 実行
input_mp3_path = "../data/chorale_correct.mp3"  # 任意の入力ファイル名
output_tsv_path = "../data/test_correct_cutstart5.tsv"  # 任意の出力ファイル名
